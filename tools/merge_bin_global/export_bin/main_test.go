...
func TestExportTableIncludesGeminiRequests(t *testing.T) {
	ctx := context.Background()
	db, err := sql.Open("sqlite3", "file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("open db: %v", err)
	...
	_, err = db.ExecContext(ctx, `
		...
		INSERT INTO gemini_api_requests (
			timestamp, session_id, model, input_tokens, output_tokens, cache_read_tokens,
			thoughts_tokens, tool_tokens, total_tokens, duration_ms, cost_usd, http_status_code,
			prompt_id, event_name, service_name, service_version
		) VALUES (
			1779868000, 'gsession-a', 'gemini-3-flash-preview', 100, 20, 50,
			5, 0, 175, 1200, 684, 200,
			'gprompt-a', 'api_response', 'gemini_cli', 'test'
		);
	`)
	if err != nil {
		t.Fatalf("seed gemini table: %v", err)
	}
	...
	if cfg.Name == "" {
		t.Fatal("gemini_api_requests config not found in tableConfigs()")
	}

	var buf bytes.Buffer
	n, err := exportTable(ctx, db, json.NewEncoder(&buf), cfg, 1779867999, 1779868001)
	if err != nil {
		t.Fatalf("export table: %v", err)
	}
	if n != 1 {
	...
	if rec.Table != "gemini_api_requests" {
		t.Fatalf("table = %q, want gemini_api_requests", rec.Table)
	}
	if rec.Row["model"] != "gemini-3-flash-preview" {
		t.Fatalf("model = %#v, want gemini-3-flash-preview", rec.Row["model"])
	}
	if rec.Row["total_tokens"] != float64(175) {
		t.Fatalf("total_tokens = %#v, want 175", rec.Row["total_tokens"])
	}
}
