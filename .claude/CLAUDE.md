...
**Three-layer lookup priority** (highest first):

1. `cfg.Pricing` (user `pricing:` block in `cc-otel.yaml`) — kept in memory, never persisted to SQLite.
2. `model_pricing` table — canonical store, seeded from `internal/pricing/embed/seed.json` on first boot, refreshed daily from LiteLLM + OpenRouter via diff writes (`Refresher` in `internal/pricing/refresher.go`).
...
- Inspect a model: `GET /api/pricing/lookup?model=glm-4.6` returns matched key, source, prices, and `is_claude`.
- Dashboard popup: top-right `live` → Server Status → **Pricing Table** row (green<48h, yellow<7d, red>7d or error).
- Backfill historical rows: `go run ./tools/recompute_cost --db <path> --table both [--apply]`. Defaults to dry-run.
- Disable daily refresher: `pricing_refresh: { enabled: false }` in `cc-otel.yaml`.

When touching pricing code, the standard build → restart → API → browser flow still applies; additionally hit `/api/pricing/lookup?model=<test>` to confirm the registry resolves and `/api/status` to confirm the popup data is populated.
