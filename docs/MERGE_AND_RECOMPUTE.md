...
6. **重复行靠 INSERT OR IGNORE**：基于主键去重。如果两库都有同一条 `request_id` 的 api_request，以 global 副本现有的为准（local 同 id 行被跳过）。
...
6. Duplicate rows are deduped by `INSERT OR IGNORE` on primary key.
